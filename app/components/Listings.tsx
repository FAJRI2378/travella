...
const Listings = () => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setListings((prev) =>
        prev.map((item) => (item.id === editingListing.id ? editingListing : item))
      );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {listings.map((listing) => (
          <div key={listing.id} className="flex flex-col">
            {/* Card Utama */}
            <motion.div
              className="border rounded-lg shadow-lg p-4 bg-white cursor-pointer transition-all hover:shadow-xl"
            ...
            >
              ...
              <div className="mt-2 text-sm text-gray-500 space-y-1">
                <p><FaTag className="text-yellow-500" /> <strong>Category:</strong> {listing.category}</p>
                <p><FaDollarSign className="text-green-500" /> <strong>Price:</strong> ${listing.price}</p>
                <p><FaBed className="text-blue-500" /> <strong>Rooms:</strong> {listing.roomCount}</p>
                <p><FaBath className="text-purple-500" /> <strong>Bathrooms:</strong> {listing.bathroomCount}</p>
                <p><FaUserFriends className="text-orange-500" /> <strong>Guests:</strong> {listing.guestCount}</p>
                <p><FaMapMarkerAlt className="text-red-500" /> <strong>Location:</strong> {listing.locationValue}</p>
                {listing.size && <p><FaRulerCombined className="text-gray-700" /> <strong>Size:</strong> {listing.size} sqft</p>}
              </div>
            </motion.div>
            ...
            <div className="mt-2">
              <div className="border p-4 rounded-md bg-gray-100">
                <div className="flex gap-4">
                  <button className="bg-yellow-500 text-white px-4 py-2 rounded-md w-full" onClick={() => handleAction('edit', listing)}>Edit</button>
                  <button className="bg-red-500 text-white px-4 py-2 rounded-md w-full" onClick={() => handleAction('delete', listing)}>Delete</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {selectedListing && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center z-50">
          <motion.div className="bg-white rounded-xl shadow-2xl p-6 w-[85%] sm:w-[480px] md:w-[500px] lg:w-[520px] relative">
            <button className="absolute top-3 right-3 text-gray-600 hover:text-gray-900" onClick={() => setSelectedListing(null)}>✖</button>
            ...
            <div className="mt-3 space-y-2 text-gray-700 text-sm">
              <p><FaTag className="text-yellow-500" /> <strong>Category:</strong> {selectedListing.category}</p>
              <p><FaDollarSign className="text-green-500" /> <strong>Price:</strong> ${selectedListing.price}</p>
              <p><FaBed className="text-blue-500" /> <strong>Rooms:</strong> {selectedListing.roomCount}</p>
              <p><FaBath className="text-purple-500" /> <strong>Bathrooms:</strong> {selectedListing.bathroomCount}</p>
              <p><FaUserFriends className="text-orange-500" /> <strong>Guests:</strong> {selectedListing.guestCount}</p>
              <p><FaMapMarkerAlt className="text-red-500" /> <strong>Location:</strong> {selectedListing.locationValue}</p>
            </div>
            <div className="mt-3">
      ...
      {editingListing && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <div className="flex justify-end mt-4">
              <button className="bg-gray-500 text-white px-4 py-2 rounded-md" onClick={() => setEditingListing(null)}>Cancel</button>
              <button className="bg-blue-600 text-white px-4 py-2 ml-2 rounded-md" onClick={handleSaveEdit}>Save</button>
            </div>
          </div>
        </div>
      )}
